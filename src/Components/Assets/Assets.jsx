...
"Zombie": (
  <div className="text-left space-y-2">
    <p className="text-l font-semibold">
    This 3D model depicts a male zombie in a detailed, realistic style.
    </p>
    <p className="text-l font-semibold">
    Designed with an idle animation setup, the character features decayed skin, tattered clothing, and a hunched posture that conveys an unsettling sense of undead movement. 
    </p>
    <p className="text-l font-semibold"> Ideal for:</p>
    <ul className="list-disc list-inside ml-4 text-l font-semibold">
      <li>Horror</li>
      <li>Survival</li>
      <li>Post-apocalyptic projects</li>
    </ul>

    <p className="text-gray-300 text-xl font-bold">Additional Information:</p>
    <ul className="list-disc list-inside ml-4 text-l font-semibold">
      <li><strong>File Format:</strong> .blend</li>
      <li><strong>Potential Uses:</strong>Suitable for game development, cinematics, AR/VR, and concept design</li>
      <li><strong>Features:</strong> Full rig with an idle animation state.Realistic textures and materials for a gritty horror vibe</li>
      <li><strong>Compatibility:</strong> Native Blender file, easily exported to other formats (FBX, OBJ)</li>
      <li><strong>Customization:</strong>Motion/pose and textures can be edited for a personalized zombie appearance.</li>
    </ul>
  </div>
),
      ...
      <div className="relative flex flex-col lg:flex-row items-start justify-between gap-8 p-6">
        <div className="w-full lg:w-2/3 flex flex-col items-start">
          ...
          {title === "Zombie" && (
            <div className="mt-4 flex space-x-2">
              <button
                onClick={() => { setIsAnimating(!isAnimating); setIsAnimatingWalk(false); }}
              ...
              <button
                onClick={() => { setIsAnimatingWalk(!isAnimatingWalk); setIsAnimating(false); }}
                className="bg-gradient-to-r from-[#fbb040] via-[#f46728] to-[#ed1c24] text-xl text-black px-4 py-2 rounded-lg flex items-center shadow-lg hover:bg-gray-600 transition"
              >
        ...
        <div className="w-full lg:w-1/3 flex flex-col justify-start" ref={detailsRef}>
          <div className="bg-gray-900 p-6 rounded-lg shadow-lg w-full text-center">
            ...
            <p className="text-xl font-semibold bg-yellow-400 text-gray-900 px-4 py-2 rounded-lg shadow-md">
              Price: {asset.price}
            </p>
            {assetTechDetails[title] && (
              <div className="bg-gray-800 p-4 rounded-lg shadow-lg mt-4">
                <h2 className="text-lg font-bold text-gray-300">Technical Details</h2>
                <p className="text-gray-400">{assetTechDetails[title]}</p>
              </div>
            )}
          </div>
        </div>
...
const ModelViewer = ({ idleModelUrl, walkModelUrl, modelHeight, rotation, cameraPosition, isAnimating, isAnimatingWalk }) => {
    ...
    <div className="relative w-full bg-gray-800 shadow-lg rounded-lg">
      <div className="relative w-full" style={{ height: modelHeight }}>
        <Canvas camera={{ position: cameraPosition, fov: 50 }}>
          <Suspense fallback={null}>
            <Model idleModelUrl={idleModelUrl} walkModelUrl={walkModelUrl} rotation={rotationMap[view]}  isAnimating={isAnimating} isAnimatingWalk={isAnimatingWalk} />
            <OrbitControls />
      ...
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2 bg-gray-900 p-1 rounded-lg">
        {Object.keys(rotationMap).map((key) => (
          <button key={key} onClick={() => setView(key)} className="bg-gray-700 text-white p-1 rounded w-16 h-12 flex items-center justify-center">
