  ...
  useEffect(() => {
    ...
    loader.load(modelUrl, (gltf) => {
      ...
      if (gltf.animations && gltf.animations.length > 0) {
        const action = mixer.current.clipAction(gltf.animations[0]); // Use the first animation
        if (isAnimating) {
          action.play();
        } else {
          action.stop();
...
const ModelViewer = ({ modelUrl, modelHeight, rotation, cameraPosition, isAnimating }) => {
      ...
      <div className="relative w-full" style={{ height: modelHeight }}>
        <Canvas camera={{ position: cameraPosition, fov: 50 }}>
          <Suspense fallback={null}>
            <Model modelUrl={modelUrl} rotation={rotationMap[view]} isAnimating={isAnimating} />
            <OrbitControls />
    ...
    <div className="bg-black min-h-screen text-white pt-20">
      <div className="relative flex flex-col lg:flex-row items-start justify-between gap-8 p-6">
        <div className="w-full lg:w-2/3 flex flex-col items-start">
          <ModelViewer 
            modelUrl={asset.modelUrl} // Ensure this points to your .glb or .gltf file
          ...
          {title === "Zombie" && (
            <div className="mt-4">
              <button
                onClick={() => setIsAnimating(!isAnimating)} // Toggle animation on button click
