...
const Model = ({ idleModelUrl, walkModelUrl, rotation, isAnimating }) => {
...
let url;
if (Array.isArray(walkModelUrl)) {
url = isAnimating ? walkModelUrl[1] : walkModelUrl[0];
} else {
url = isAnimating ? walkModelUrl : idleModelUrl;
}
...
}, [idleModelUrl, walkModelUrl, isAnimating]);
...
object={model}
rotation={rotation}
scale={[7, 7, 7]} // Using the scale from the zombie model data
/>
) : null;
};
...
const { title, extendedDescription, price, modelUrl, walkModelUrl, rotation = [0, 0, 0], technical, scale } = asset;
 
// Check if the asset has both idle and walk animations
// Only show controls if walkModelUrl is an array with at least 2 elements
const hasAnimations = Array.isArray(walkModelUrl) && walkModelUrl.length >= 2;
 
// Use the asset's scale or default to [7, 7, 7] for the zombie
const modelScale = scale || [7, 7, 7];
...
console.log("walkModelUrl:", walkModelUrl);
...
{/* Only show animation controls for models with both idle and walk animations */}
{hasAnimations && (
...
<button
onClick={() => {
setIsAnimating(selectedAnimation === 'walk');
}}
